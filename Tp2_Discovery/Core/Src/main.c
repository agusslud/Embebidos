...
/* USER CODE BEGIN PV */
volatile uint32_t t_presionado_ms = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  MX_TIM4_Init();
  /* USER CODE BEGIN 2 */
  HAL_TIM_Base_Start_IT(&htim2); // Parte 1
  HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_1); // Parte 2
  HAL_TIM_IC_Start_IT(&htim4, TIM_CHANNEL_1); // Parte 3
  /* USER CODE END 2 */
...
void Dimmer_Task(void) {
	static uint32_t t_dimmer = 0;
	static uint8_t duty = 0;
	static int8_t dir = 1; // 1: Aumenta Brillo, -1: Atenúa Brillo
